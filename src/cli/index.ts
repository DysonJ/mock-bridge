  ...
  try {
    ...
    if (fs.existsSync(pkgPath)) {
      ...
      if (portMatch) {
        ...
        console.log(
          chalk.gray(
            `🔍 Auto-detected app port from package.json: ${detectedPort}`,
          ),
        );
      }
    }
...
function validateConfig(config: MockShopifyAdminConfig): void {
  ...
  if (!config.appUrl) {
    errors.push(
      "App URL is required (provide as argument or set in config file)",
    );
  }
  ...
  if (errors.length > 0) {
    ...
    console.log(
      chalk.gray(
        "   npx mock-bridge http://localhost:3000 --client-id your-client-id",
      ),
    );
...
async function startCommand(options: CLIConfig): Promise<void> {
  try {
    ...
    if (finalConfig.clientId === STANDARD_MOCK_CLIENT_ID) {
      ...
      console.log(
        chalk.gray(
          "   For production-like testing, provide your real client ID:",
        ),
      );
      console.log(
        chalk.gray(
          "   npx @getverdict/mock-bridge http://localhost:3000 --client-id your-real-id",
        ),
      );
      console.log();
    }
    ...
    console.log(
      chalk.green.bold("🎉 Mock Shopify Admin Started Successfully!"),
    );
    console.log();
    console.log(chalk.white.bold("📍 URLs:"));
    console.log(
      chalk.gray("   "),
      chalk.white("Mock Admin:"),
      chalk.green(`http://localhost:${finalConfig.port}`),
    );
    console.log(
      chalk.gray("   "),
      chalk.white("Your App:"),
      chalk.cyan(finalConfig.appUrl),
    );
    console.log();
    console.log(chalk.white.bold("🎯 Next Steps:"));
    console.log(
      chalk.gray("   1. Make sure your app is running at"),
      chalk.cyan(finalConfig.appUrl),
    );
    console.log(
      chalk.gray("   2. Open"),
      chalk.green(`http://localhost:${finalConfig.port}`),
      chalk.gray("in your browser"),
    );
    ...
    if (finalConfig.proxy) {
      console.log(
        chalk.gray("   ✅ Same-origin proxy mode (Cypress compatible)"),
      );
      console.log();
      console.log(chalk.white.bold("🔀 Proxy Mode:"));
      console.log(
        chalk.gray("   App is proxied at"),
        chalk.green(`http://localhost:${finalConfig.port}/__proxy/`),
      );
      console.log(
        chalk.gray("   Iframe is same-origin — Cypress can access it directly"),
      );
    }
    console.log();
...
async function initCommand(): Promise<void> {
  console.log(
    chalk.blue.bold("🔧 Creating Shopify App Bridge Mock configuration..."),
  );
...
`;
  ...
  if (fs.existsSync(configPath)) {
    console.log(
      chalk.yellow("⚠️  Configuration file already exists:"),
  ...
  console.log(
    chalk.gray("   1. Edit"),
    chalk.cyan("mock.config.js"),
    chalk.gray("with your app settings"),
  );
  console.log(
    chalk.gray("   2. Set"),
    chalk.cyan("SHOPIFY_API_KEY"),
    chalk.gray("environment variable"),
  );
  console.log(
    chalk.gray("   3. Run"),
    chalk.green("npx mock-bridge"),
    chalk.gray("to start"),
  );
  console.log();
...
program
  ...
  .option(
    "-i, --client-id <id>",
    "Your Shopify app's client ID (optional, defaults to development ID)",
  )
  .option(
    "-s, --client-secret <secret>",
    "Mock client secret (development only)",
  )
  ...
  .option(
    "--proxy",
    "Proxy app through mock-bridge for same-origin iframe (enables Cypress support)",
    false,
  )
  .option(
    "--app-name <name>",
    "Display name shown in the mock admin navigation sidebar",
  )
